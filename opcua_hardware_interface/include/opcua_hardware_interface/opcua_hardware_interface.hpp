...
class OPCUAHardwareInterface : public hardware_interface::SystemInterface
{
  ...
  private:
    ...
    std::vector<StateInterfaceUANode>
        state_interfaces_nodes; // Contains the node IDs cooresponding to the state interfaces.
    std::vector<CommandInterfaceUANode>
        command_interfaces_nodes; // Contains the node IDs cooresponding to the command interfaces.
